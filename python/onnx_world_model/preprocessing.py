# @agent-file
# @agent-purpose: Turns user prompts and media into the exact tensors a Mobius package expects: it renders chat templates, tokenizes text, packs latent tokens, and assembles reasoner and world-model inputs from the package manifest.
# @agent-public-api: PreparedReasonerInputs, PreparedConditioning, PreparedWorldInputs, TextPreprocessor, GeneratorPromptPacker, WorldModelPreprocessor, pack_latent_tokens, unpack_latent_tokens, and the media re-exports RawImage, RawVideo, conditioning_resample, ConditioningFramePreprocessor, ImagePreprocessor, PackedImage, PackedImagePreprocessor, PreparedVideo, PackedVideoPreprocessor
# @agent-invariants: Chat templates are rendered in a Jinja2 immutable sandbox, so a template that reaches outside its data raises SecurityError. pack_latent_tokens and unpack_latent_tokens are exact inverses of the runtime video unpatchify layout. Preparation is driven by the package manifest and its config files, never by hard-coded component names. The media symbols are re-exported so that `onnx_world_model.preprocessing` stays the single documented import path.
# @agent-side-effects: Reads tokenizer, chat-template, preprocessor, and scheduler configuration files from the package directory, and reads caller-supplied image and video files through the media module.
...
from onnx_world_model.media import (
