/**
 * @agent-file
 * @agent-purpose: Parses pipeline.json into a PipelineManifest and loads a Mobius package: it resolves component files inside the package root, opens their ONNX sessions, and builds the shareable Pipeline.
 * @agent-public-api: Endpoint::Parse, Endpoint::qualified, PipelineManifest::Parse, PipelineManifest::Load, PipelineManifest::SupportedCapabilities, PipelineManifest accessors, PipelineManifest::Component, PipelineManifest::Input, PipelineManifest::Output, PipelinePackage::PipelinePackage, PipelinePackage::Load, PipelinePackage accessors, Pipeline::Pipeline, Pipeline::Load
 * @agent-invariants: Only Mobius pipeline schema major version 1 is accepted, and unknown JSON fields are rejected rather than ignored. Every declared path stays inside the package root, and each component ONNX signature must agree with the manifest. Parsing populates the manifest and then hands it to detail::ValidateManifest, so a PipelineManifest that exists has already passed semantic validation. Pipeline owns component sessions through a shared_ptr so copies share one set of ONNX sessions.
